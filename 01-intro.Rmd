...
```{r}
# ===== 6.1 Variable numerica vs categorica =====
suppressPackageStartupMessages({
  library(dplyr)
  library(ggplot2)
  library(forcats)
...
# ---- Paleta coherente ----
azul_main   <- "#4A90E2"  # rellenos/barras principales
azul_borde  <- "#1F5AA6"  # lineas/bordes destacados
gris_linea  <- "#5A5A5A"  # contornos neutros
rojo_out    <- "#D94E4E"  # SOLO para outliers si aplica

# 0) Asegurar variable acotada (si aun no existe)
if (!"tasa_fin_cap" %in% names(df_2)) {
  df_2 <- df_2 %>%
    mutate(tasa_fin_cap = pmin(pmax(tasa_fin, 0), 100))
}

# 1) Seleccionar TOP paises por cobertura (N paises con mas años)
N_TOP <- 12  # ajusta si quieres mas/menos
top_paises <- df_2 %>%
  filter(!is.na(tasa_fin_cap), !is.na(pais), !is.na(anio),
         anio >= 2000, anio <= 2022) %>%
  distinct(pais, anio) %>%
  count(pais, name = "n_anios") %>%
  arrange(desc(n_anios), pais) %>%
  slice_head(n = N_TOP) %>%
  pull(pais)
...
df_top <- df_2 %>%
  filter(pais %in% top_paises,
         !is.na(tasa_fin_cap),
         anio >= 2000, anio <= 2022) %>%
  mutate(pais = fct_reorder(pais, tasa_fin_cap, .fun = median, .desc = TRUE))

# 2) Boxplot por pais (para distribuciones comparables)
p_box <- ggplot(df_top, aes(x = pais, y = tasa_fin_cap)) +
  geom_boxplot(fill = azul_main, color = gris_linea, outlier.colour = rojo_out, outlier.alpha = 0.8) +
  stat_summary(fun = median, geom = "point", size = 1.6, color = azul_borde) +  # resalta la mediana
  coord_flip() +
  labs(
    ...
    caption = "Punto azul oscuro: mediana. Outliers resaltados en rojo."
  ) +
...
# 3) Violin + boxplot (opcional, para ver forma + cuantiles)
p_violin <- ggplot(df_top, aes(x = pais, y = tasa_fin_cap)) +
  geom_violin(fill = scales::alpha(azul_main, 0.6), color = azul_borde) +
  geom_boxplot(width = 0.15, fill = "white", color = gris_linea, outlier.colour = rojo_out, outlier.alpha = 0.7) +
  coord_flip() +
  labs(
    title = "Distribucion por pais: violin + boxplot",
